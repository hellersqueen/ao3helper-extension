...
async function startBrowserProcess(browser, browserArgs, url) {
	...
	if (process.platform === "darwin" && (!preferredOSXBrowser || supportedChromiumBrowsers.includes(preferredOSXBrowser))) try {
		...
		if (openedBrowser) {
			await execAsync(`osascript openChrome.applescript "${url}" "${openedBrowser}"`, { cwd: join(VITE_PACKAGE_DIR, "bin") });
			return true;
		}
	} catch {}
...
const installUserPath = urlPrefix + "install.user.js";
...
const serverFactory = (getOption) => {
	...
	return {
		...
		async config(userConfig) {
			option = await getOption();
			for (const [k, v] of Object.entries(option.userscript.name)) Reflect.set(option.userscript.name, k, option.server.prefix(v));
			option.userscript.grant.add("*");
			monkeyWindowKey = `__monkeyWindow-` + simpleHash(await finalMonkeyOptionToComment(option, /* @__PURE__ */ new Set(), "serve"));
			return {
				preview: {
		...
		transform(code, id) {
			if ((id.includes("vite-plugin-monkey") || id.includes("/node_modules/")) && code.includes("__MONKEY_WINDOW_KEY__")) return code.replaceAll("__MONKEY_WINDOW_KEY__", JSON.stringify(monkeyWindowKey));
		},
		async configureServer(server) {
			server.middlewares.use((_, res, next) => {
				const name = "access-control-allow-private-network";
				if (!res.hasHeader(name)) res.setHeader(name, "true");
				next();
			});
			...
			server.middlewares.use(async (req, res, next) => {
				if (!(req.method === "GET" && req.url && req.url.startsWith(urlPrefix))) return next();
				const url = new URL(req.url, localOrigin);
				if (url.pathname === "/__vite-plugin-monkey.install.user.js") {
					setScriptHeader(res);
					...
					res.end([
						...
						stringifyFunction(serverInjectFn, new URL(entryPath, origin).href, monkeyWindowKey),
						""
					].join("\n\n"));
					...
					const entryUrls = (option.server.mountGmApi ? [gmApiPath] : []).concat(results.map((v) => {
						return v.src || `${pullPath}?text=${Buffer.from(v.text, "utf-8").toString("base64url")}`;
					}));
					const realEntry = path.isAbsolute(option.entry) ? normalizePath(path.relative(viteConfig.root, option.entry)) : option.entry;
					const entryUrl = new URL(realEntry, localOrigin);
					entryUrls.push(entryUrl.pathname + entryUrl.search);
					res.end(entryUrls.map((s) => `import ${JSON.stringify(s)};`).join("\n"));
				} else if (url.pathname === pullPath) {
					setScriptHeader(res);
					res.end(Buffer.from(url.searchParams.get("text") ?? "", "base64url").toString("utf-8"));
				} else if (url.pathname === gmApiPath) {
					setScriptHeader(res);
					if (option.server.mountGmApi) res.end(stringifyFunction(mountGmApiFn, monkeyWindowKey, gmIdentifiers.concat()));
					else res.end("");
				} else next();
			});
			if (option.server.open) {
				const cacheUserPath = `node_modules/.vite/__vite-plugin-monkey.cache.${simpleHash(viteConfig.configFile)}.user.js`;
				let cacheComment = "";
				if (await existFile(cacheUserPath)) cacheComment = (await fs.readFile(cacheUserPath)).toString("utf-8");
				else await fs.mkdir(path.dirname(cacheUserPath)).catch(() => {});
				const newComment = await finalMonkeyOptionToComment(option, /* @__PURE__ */ new Set(), "serve");
...
//#endregion
//#region src/node/plugins/style.ts
const styleFactory = (_getOption, pluginOption) => {
	...
	return {
		...
		load(id) {
			...
			if (id.endsWith(cssIdSuffix)) return getStyleModule(id.substring(0, id.length - cssIdSuffix.length), isServe);
		}
	};
};
const styleQuery = "?style";
const cssIdSuffix = ".plugin-monkey-style";
const getStyleModule = (cssId, isServe) => {
...
//#endregion
//#region src/node/plugins/virtualHtml.ts
const virtualHtmlFactory = () => {
	return {
...
//#endregion
//#region src/node/cdn.ts
var cdn_exports = /* @__PURE__ */ __exportAll({
	bdstatic: () => bdstatic,
	bootcdn: () => bootcdn,
	cdnjs: () => cdnjs,
	elemecdn: () => elemecdn,
...
*/
const jsdelivr = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		const p = pathname || resolveName;
		if (p) return `https://cdn.jsdelivr.net/npm/${name}@${version}/${p}`;
		else return `https://cdn.jsdelivr.net/npm/${name}@${version}`;
	}];
};
/**
...
* @see https://www.jsdelivr.com/features
*/
const jsdelivrFastly = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		const p = pathname || resolveName;
		if (p) return `https://fastly.jsdelivr.net/npm/${name}@${version}/${p}`;
		else return `https://fastly.jsdelivr.net/npm/${name}@${version}`;
	}];
};
/**
...
* @see https://unpkg.com/
*/
const unpkg = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		const p = pathname || resolveName;
		if (p) return `https://unpkg.com/${name}@${version}/${p}`;
		else return `https://unpkg.com/${name}@${version}`;
	}];
};
/**
* `https://fastly.jsdelivr.net/npm/${name}@${version}/${pathname}`
...
*/
const bootcdn = (exportVarName = "", pathname = "") => {
	console.warn("[plugin-monkey] bootcdn will return virus-infected code. Please stop using it and switch to other sources. now it will return jsdelivr url.");
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		return `https://fastly.jsdelivr.net/npm/${name}@${version}/${pathname || resolveName}`;
	}];
};
/**
* `https://fastly.jsdelivr.net/npm/${name}@${version}/${pathname}`
...
*/
const staticfile = (exportVarName = "", pathname = "") => {
	console.warn("[plugin-monkey] staticfile will return virus-infected code. Please stop using it and switch to other sources. now it will return jsdelivr url.");
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		return `https://fastly.jsdelivr.net/npm/${name}@${version}/${pathname || resolveName}`;
	}];
};
...
*/
const cdnjs = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		return `https://cdnjs.cloudflare.com/ajax/libs/${name}/${version}/${pathname || resolveName}`;
	}];
};
/**
...
*/
const zhimg = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		return `https://unpkg.zhimg.com/${name}@${version}/${pathname || resolveName}`;
	}];
};
/**
...
* @param exportVarName cdn-exportVarName or resourceName
*/
const elemecdn = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		return `https://npm.elemecdn.com/${name}@${version}/${pathname || resolveName}`;
	}];
};
/**
...
* @param exportVarName cdn-exportVarName or resourceName
*/
const bdstatic = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		return `https://code.bdstatic.com/npm/${name}@${version}/${pathname || resolveName}`;
	}];
};
/**
...
* @param exportVarName cdn-exportVarName or resourceName
*/
const npmmirror = (exportVarName = "", pathname = "") => {
	return [exportVarName, (version, name, _importName = "", resolveName = "") => {
		const p = pathname || resolveName;
		if (p) return `https://registry.npmmirror.com/${name}/${version}/files/${p}`;
		else return `https://registry.npmmirror.com/${name}/${version}/files`;
	}];
};
//#endregion
//#region src/node/utils/option.ts
const resolvedOption = async (pluginOption) => {
	if (pluginOption.format) setTimeout(() => {
		console.log("[vite-plugin] `format` option is deprecated, use `align` and `generate` instead");
	});
	const build = pluginOption.build ?? {};
	const { externalResource = {} } = build;
	const externalResource2 = {};
	for (const [k, v] of Object.entries(externalResource)) if (typeof v == "string") externalResource2[k] = {
		resourceName: () => k,
		resourceUrl: () => v
	};
	else if (typeof v == "function") externalResource2[k] = {
		resourceName: () => k,
		resourceUrl: v
	};
		...
		if (typeof resourceName == "string") resourceName2 = (pkg) => resourceName || pkg.importName;
		else resourceName2 = (pkg) => resourceName(pkg.version, pkg.name, pkg.importName, pkg.resolveName);
		if (typeof resourceUrl == "string") resourceUrl2 = () => resourceUrl;
		else resourceUrl2 = (pkg) => resourceUrl(pkg.version, pkg.name, pkg.importName, pkg.resolveName);
		externalResource2[k] = {
		...
		if (typeof resourceUrl == "string") resourceUrl2 = () => resourceUrl;
		else resourceUrl2 = resourceUrl;
		if (typeof resourceName == "string") resourceName2 = () => resourceName;
		else if (typeof resourceName == "function") resourceName2 = resourceName;
		if (typeof nodeLoader == "function") nodeLoader2 = nodeLoader;
		else if (typeof nodeLoader == "string") nodeLoader2 = () => nodeLoader;
		externalResource2[k] = {
			resourceName: resourceName2,
			resourceUrl: resourceUrl2,
			loader,
	...
	if (typeof prefix == "function") prefix2 = prefix;
	else if (typeof prefix == "string") prefix2 = () => prefix;
	else if (prefix === false) prefix2 = (name) => name;
	const externalGlobals2 = build?.externalGlobals ?? {};
	const externalGlobals = [];
	if (externalGlobals2 instanceof Array) externalGlobals2.forEach((s) => externalGlobals.push(s));
	else Object.entries(externalGlobals2).forEach((s) => externalGlobals.push(s));
	const projectPkg = await getProjectPkg();
	const { grant = [], $extra = [] } = pluginOption.userscript ?? {};
	let { name = {}, description = {}, "exclude-match": excludeMatch = [], match = [], exclude = [], include = [], antifeature = [], require = [], connect = [], webRequest = [] } = pluginOption.userscript ?? {};
	if (typeof name == "string") name = { "": name };
	else if (!("" in name)) name = {
		...projectPkg.name ? { "": projectPkg.name } : {},
		...name
	};
	if (typeof description == "string") description = { "": description };
	else if (!("" in description) && projectPkg.description) description = {
		"": projectPkg.description,
		...description
	};
	if (!(excludeMatch instanceof Array)) excludeMatch = [excludeMatch];
	...
	if (!(antifeature instanceof Array)) antifeature = [antifeature];
	if (!(require instanceof Array)) require = [require];
	if (!(connect instanceof Array)) connect = [connect];
	if (!(webRequest instanceof Array)) webRequest = [webRequest];
	const grantSet = /* @__PURE__ */ new Set();
	if (typeof grant == "string") grantSet.add(grant);
	else if (grant instanceof Array) grant.forEach((s) => grantSet.add(s));
	const extra = [];
	($extra instanceof Array ? $extra : Object.entries($extra)).forEach(([k, v]) => {
		extra.push([k, ...v instanceof Array ? v : [v]]);
	});
	const { icon64, icon64URL, icon, iconURL, namespace, version = projectPkg.version, author = projectPkg.author, copyright, downloadURL, defaulticon, contributionURL, updateURL, supportURL = projectPkg.bugs, homepageURL = projectPkg.homepage, homepage = projectPkg.homepage, website, license = projectPkg.license, incompatible, source = projectPkg.repository, resource = {}, noframes = false, "run-at": runAt, "inject-into": injectInto, contributionAmount, compatible, sandbox, tag, unwrap = false } = pluginOption.userscript ?? {};
	const { fileName = projectPkg.name + ".user.js" } = build;
	let { metaFileName } = build;
	if (typeof metaFileName == "string") {
		const t = metaFileName;
		metaFileName = () => t;
	} else if (metaFileName === true) metaFileName = () => fileName.replace(/\.user\.js$/, ".meta.js");
	else if (metaFileName === false) metaFileName = void 0;
...
var node_default = (pluginOption) => {
	...
	return factorys.map((f) => f(getOption, pluginOption)).filter(Boolean);
};
const util = {
	dataUrl,
	unimportPreset: {
...
//#endregion
export { cdn_exports as cdn, node_default as default, util };
