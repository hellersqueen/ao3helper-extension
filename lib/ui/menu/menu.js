...
import { openAO3HPanel } from '../panel/lazy-panel.js';
...
const W = (typeof unsafeWindow !== 'undefined') ? unsafeWindow : window;
  ...
  log.debug('Initial state - modulesReady:', modulesReady, ', grouperReady:', grouperReady);
  ...
  const itemToggle = (label, flagKey, current, moduleName = null) => {
    log.debug('🧪 itemToggle called with:', { label, flagKey, current, moduleName });
    ...
    //   log.debug('Using uiComponents.itemToggle (no settings button support yet)');
    //   return uiComponents.itemToggle(label, flagKey, current, NS);
    // }
    
    log.debug('🧪 Creating fallback toggle with button');
    ...
    log.debug('Creating icon button for module:', moduleName);
    ...
    if (window.AO3H_IconButton && typeof window.AO3H_IconButton.createSettingsButton === 'function') {
      ...
      iconBtn.onclick = function(e) {
        ...
        log.debug('Opening panel for module:', modName);
    ...
    log.debug('Created module item with settings button:', li);
    return li;
  };
  ...
  function openMenu(){
    ...
    if (!modulesReady) {
      log.debug('Menu blocked: modules not ready yet');
      return;
    }
    
    // Don't open menu if modules are still registering
    if (modulesRegistering > 0) {
      log.debug('Menu blocked: modules still registering (' + modulesRegistering + ' remaining)');
      return;
    }
    
    // Don't open menu if grouper hasn't finished organizing modules yet
    if (!grouperReady) {
      log.debug('Menu blocked: waiting for menu-grouper to finish organizing');
      return;
    }
    
    // Don't open menu if we're currently rebuilding/regrouping
    if (rebuilding) {
      log.debug('Menu blocked: currently rebuilding/regrouping');
      return;
    }
    
    log.debug('Opening menu (modules ready:', modulesReady, ', grouper ready:', grouperReady, ')');
  ...
  function buildMenu(){
      ...
      if (toggleEl && !toggleEl.hasAttribute('data-ao3h-click-configured')) {
        on(toggleEl, 'click', (e)=>{ 
          ...
          if (!modulesReady || !grouperReady) {
            log.debug('Click blocked: waiting for modules and grouper to be ready');
            return;
          }
    ...
    on(menuUL, 'click', async (e)=>{
      ...
      if (settingsBtn) {
        e.preventDefault();
        e.stopPropagation();
        log.debug('⚙️ Settings button clicked');
        
        const moduleName = settingsBtn.dataset.moduleName;
        log.debug('Opening panel for module:', moduleName);
  ...
  // Same default-resolution rule as fillMenu(): real modules fall back to
  // their own meta.enabledByDefault, custom items to their declared defaultOn.
  function defaultForFlag(flagKey){
    const mods = (Modules && Modules.all ? Modules.all() : []);
    const mod = mods.find(m => m.enabledKey === flagKey || m.enabledKeyAlt === flagKey);
    if (mod) return !!mod.meta?.enabledByDefault;
    const item = customItems.find(it => it.type === 'toggle' && it.flagKey === flagKey);
  ...
  function setupModuleReadyListeners() {
    ...
    const handleModuleLoading = (e) => {
      modulesRegistering++;
      log.debug('Module loading:', e.detail?.name, '(total registering:', modulesRegistering + ')');
    };
    
    const handleModuleRegistered = (e) => {
      modulesRegistering = Math.max(0, modulesRegistering - 1);
      log.debug('Module registered:', e.detail?.name, '(remaining:', modulesRegistering + ')');
      
      // If all modules are done registering, allow menu to rebuild
      if (modulesRegistering === 0 && menuUL) {
        log.debug('All modules registered, menu ready for interaction');
      }
    };
    ...
    const markModulesReady = () => {
      ...
      log.debug('Modules are ready, enabling menu interactions');
      modulesReady = true;
      
      // Rebuild menu to reflect current module state — go through
      // AO3H.menu.rebuild() (not the local fillMenu() directly) so that
      // menu-grouper's rebuild hook runs too and recreates the group
      // containers fillMenu()'s innerHTML wipe just destroyed.
      if (menuUL) AO3H.menu.rebuild();
    };
    
    const markGrouperReady = () => {
      if (grouperReady || grouperSignaled) return; // Already marked as ready or signaled
      
      log.debug('Menu-grouper has finished organizing');
      ...
      log.debug('Menu interactions enabled (grouper ready)');
    };
    ...
    setTimeout(() => {
      if (!modulesReady) {
        ...
        if (hasModules) {
          log.debug('Modules detected via fallback check, enabling menu');
          markModulesReady();
        } else {
          // Étape 317 : hasJQuery importé (avant : window.AO3H_Common, absent côté Vite)
          if (hasJQuery()) {
            log.debug('jQuery detected, assuming modules will load soon');
          }
        }
      }
      ...
      if (!grouperReady) {
        ...
        if (!menuGrouper) {
          log.debug('No menu-grouper detected, allowing menu interaction');
          markGrouperReady();
        } else {
          // Grouper exists but may not have finished - give it more time
          log.debug('Menu-grouper detected but not ready yet, waiting...');
          setTimeout(() => {
            if (!grouperReady && !grouperSignaled) {
              log.debug('Timeout waiting for grouper, enabling menu anyway');
              grouperReady = true; // Direct activation for timeout case
            } else if (grouperSignaled && !grouperReady) {
              log.debug('Timeout reached but grouper already signaled, waiting for its delay to complete');
            }
    ...
    try {
      if (AO3H.moduleLoader && typeof AO3H.moduleLoader.getStatus === 'function') {
        ...
        if (moduleStatus && moduleStatus.length > 0) {
          log.debug('Module loader reports ready modules, enabling menu');
          markModulesReady();
        }
      }
      ...
      if (isPageFullyReady()) {
        ...
        if (hasModules && !modulesReady) {
          log.debug('Page fully ready with modules, enabling menu immediately');
          markModulesReady();
        }
      }
    } catch (err) {
...
export function initMenu() {
  onReady(async ()=>{
    try {
      ...
      if (ao3Common && typeof ao3Common.waitForAO3Ready === 'function') {
        log.debug('Waiting for AO3 libraries (jQuery, CSRF)...');
        ...
        if (result.timeout) {
          ...
          log.debug('DOM ready, building menu');
        }
      }
      ...
      if (AO3H.DEBUG) {
        ...
        log.debug(`Integration status: ${diagnostics.join(', ')}`);
      }
      ...
      document.addEventListener(`${NS}:flags-updated`, () => {
        ...
        menuUL.querySelectorAll('a[data-flag]').forEach(a => {
          ...
          const on = !!Flags.get(flagKey, defaultForFlag(flagKey));
  ...
  async function openPanelModal(moduleName) {
    log.debug('Creating panel modal for:', moduleName);
    ...
    `;
    log.debug('✅ Panel displayed with test content');
