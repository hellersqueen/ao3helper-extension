...
import { TABS, GROUPS } from '../panel/tab-registry.js';
import { getLogger } from '../../utils/logger.js';
const log = getLogger('menu-grouper');
...
// GROUPS is now imported directly from tab-registry.js (single source of
// truth) — see its own "GROUPS format expected by menu-grouper.js" comment.
const getGroups = () => GROUPS;
    ...
    const isReload = () => {
      try {
        ...
        if (navEntries.length > 0) {
          return /** @type {PerformanceNavigationTiming} */ (navEntries[0]).type === 'reload';
        }
  ...
  function createSubmenu(label){
    ...
    a.addEventListener('click', (e)=>{
      e.preventDefault();
      // Prevent AO3's delegated dropdown handlers from undoing this state.
      e.stopPropagation();
      toggle();
    });
    ...
    document.addEventListener('pointerdown', (ev)=>{
      ...
      if (!root) return;
      const insideRoot = root.contains(/** @type {Node} */ (ev.target));
      const insideThis = li.contains(/** @type {Node} */ (ev.target));
  ...
  function createManagerPanelButton() {
    ...
    a.addEventListener('click', (e) => {
      ...
      log.debug('Manager Panel button clicked');
  ...
  function createEmptyGroups() {
    log.debug('Creating empty group structure...');
    
    const menuUL = /** @type {HTMLElement} */ (document.querySelector(SEL.menuUL));
    if (!menuUL) {
    ...
    log.debug(`Creating ${groups.length} empty groups:`, groups.map(g => g.label));
    ...
    log.debug('Manager Panel button added to menu');
    ...
    log.debug(`Hiding ${topToggleAs.length} toggles until population`);
    ...
    log.debug('Empty group structure created, all toggles hidden, menu revealed');
  }
...
const _tabDefs = TABS || [];
  ...
  function decideGroup(mod){
    ...
    if (mod?.meta?.group) {
      ...
      if (groupLabel) {
        log.debug(`Module "${name}" group key "${mod.meta.group}" → label "${groupLabel}"`);
        return groupLabel;
      }
  ...
  function populateGroups(){
    log.debug('Populating groups with menu items...');
    ...
    const topAs = /** @type {HTMLElement[]} */ (Array.from(document.querySelectorAll(SEL.topLevelA)));
    log.debug(`Found ${topAs.length} top-level menu items`);
    if (!topAs.length) return;

    // Get all registered modules and create flag -> module map
    const mods = (Modules?.all?.() ?? []);
    log.debug(`Found ${mods.length} registered modules:`, mods.length > 0 ? mods.map(m => m.name) : '(none yet)');
    ...
    log.debug(`Flag mappings:`, byFlag.size > 0 ? Array.from(byFlag.keys()) : '(none yet)');
    ...
    const topToggleAs = topAs.filter(a => a.matches('[data-flag]'));
    log.debug(`Found ${topToggleAs.length} toggle items with data-flag:`, topToggleAs.length > 0 ? topToggleAs.map(a => a.dataset.flag) : '(none)');
    ...
    for (const a of topToggleAs){
      ...
      if (mod) {
        ...
        log.debug(`Module "${mod.name}" (${flagKey}) → Group: "${group}"`);
      } else {
        ...
        log.debug(`Module not loaded yet for flag: ${flagKey}, inferred group: "${group}"`);
      }
    ...
    log.debug(`Populated ${groupedCount} modules into groups`);
    
    // Signal that grouping is complete
    try {
      log.debug('Signaling grouper ready state');
  ...
  function reapplySubmenuState(){
    log.debug('🔄 Reapplying submenu state...');
    const subs = document.querySelectorAll(`li[data-ao3h-submenu="1"]`);
    log.debug('Found', subs.length, 'potential submenus to check');
    ...
    subs.forEach(li => {
      ...
      log.debug('Group', label, '- currently:', currentlyOpen, ', should be:', shouldBeOpen);
      
      if (currentlyOpen !== shouldBeOpen) {
        ul.classList.toggle('open', shouldBeOpen);
        header.setAttribute('aria-expanded', String(shouldBeOpen));
        log.debug('✓ Updated', label, 'to', shouldBeOpen);
      }
    });
    log.debug('✓ Reapply complete -', processed, 'groups processed');
  }
  ...
  function hookMenuRebuild(){
    ...
    if (api && typeof api.rebuild === 'function' && !api.__ao3hGroupPatch){
      log.debug('Hooking menu rebuild function...');
      const orig = api.rebuild.bind(api);
      api.rebuild = function(){
        log.debug('Menu rebuild triggered, recreating groups...');
        const r = orig();
        ...
        if (root && root.classList.contains('open')) {
          setTimeout(()=>{ 
            log.debug('Repopulating groups after menu rebuild...');
            populateGroups(); 
            syncCloneStates(); 
            reapplySubmenuState(); 
          }, 0);
      ...
      log.debug('Menu rebuild already hooked');
    }
  }
  ...
  function boot(){
    ...
    log.debug('Menu found, initializing grouper...');
  ...
  function waitForMenu(){
    ...
    if (checkMenuAvailable()) {
      log.debug('Menu already available, starting...');
      setTimeout(boot, 50);
      return;
    }
    
    log.debug('Waiting for menu to be available...');
    ...
    const startGrouper = () => {
      ...
      if (!checkMenuAvailable()) {
        log.debug('Menu not ready yet, waiting more...');
        setTimeout(() => {
      ...
      started = true;
      log.debug('Menu available, starting grouper...');
      setTimeout(boot, 50);
    };
    ...
    const pollForReadiness = () => {
      ...
      if (checkMenuAvailable()) {
        log.debug('Menu found via polling after ' + (attempts * 200) + 'ms, starting...');
        startGrouper();
...
export const menuGrouper = {
  ...
  rebuild: () => {
    log.debug('External rebuild requested');
