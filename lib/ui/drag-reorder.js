 ...
 * @param {(li: HTMLElement) => boolean} [opts.filter] - Ne rendre draggables
 *   que les items pour lesquels ce prédicat est vrai (défaut : tous). Utile
 *   quand la liste contient des <li> qui ne doivent pas être réordonnables.
 * @returns {() => void} Cleanup : retire poignées, listeners et classes.
 *   Si getItemKey est fourni, porte aussi `.resetToOriginal()` — remet la
 *   liste dans l'ordre constaté à l'appel de makeListReorderable (utile pour
 *   un bouton « Reset order » ; ne retire pas les poignées/listeners, juste
 *   l'ordre — appeler cleanup() séparément pour désactiver le drag & drop).
 */
export function makeListReorderable(list, {
  ...
  let items = Array.from(list.querySelectorAll(itemSelector));
  if (filter) items = items.filter(filter);
  if (items.length < 2) return () => {};

  const originalOrder = getItemKey ? items.map(getItemKey).filter(Boolean) : null;
  ...
  cleanup.resetToOriginal = function resetToOriginal() {
    if (!originalOrder) return;
    applySavedOrder(list, originalOrder, getItemKey, itemSelector);
    onOrderChanged?.(currentOrder());
  };
