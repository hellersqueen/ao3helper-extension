...
import { closePanel } from './panel-ui.js';
...
const panelFooterState = {
  ...
  function getEnabledModuleCount() {
    ...
    const modules = window.AO3H?.modules?.all?.() || _W.AO3H?.modules?.all?.() || [];
...
export function setupCloseHandlers(backdrop, box) {
    ...
    console.log('[AO3H][tab-system] ✅ Close handlers attached');
  };
...
export function setupBulkActions(tabContainer, tabId) {
    ...
    // Wire Enable All / Disable All from the top search bar area
    const box = panelBody?.closest(`.${NS}-panel-box`);
    box?.querySelectorAll(`.${NS}-bulk-actions-buttons .ao3h-panel-action-btn[data-action]`).forEach(btn => {
      // Remove old listener by cloning
      const fresh = btn.cloneNode(true);
      btn.replaceWith(fresh);
      fresh.addEventListener('click', function() {
        const action = this.getAttribute('data-action');
        const checkboxes = tabContainer.querySelectorAll('.ao3h-quick-enable-checkbox');
        const shouldCheck = action === 'enable-all';
        checkboxes.forEach(cb => {
          cb.checked = shouldCheck;
          cb.dispatchEvent(new Event('change', { bubbles: true }));
        });
        console.log(`[AO3H][tab-system] ${action}: ${checkboxes.length} modules`);
      });
    });
    console.log('[AO3H][tab-system] ✅ Bulk actions setup for tab:', tabId);
  };
...
export function setupChipPickers(container) {
    ...
    pickerRows.forEach(row => {
      ...
      function addChip(value, text, updateArray = true) {
        ...
        chip.querySelector('button').addEventListener('click', () => {
          ...
          updateDataValue();
          console.log(`[AO3H][chip-picker:${pickerType}] Removed:`, value);
        });
      ...
      function addItem() {
        ...
        if (selectedItems.includes(value)) {
          console.log(`[AO3H][chip-picker:${pickerType}] Already added:`, value);
          return;
        }
        ...
        console.log(`[AO3H][chip-picker:${pickerType}] Added:`, value, '| Total:', selectedItems.length);
      }
      ...
      console.log(`[AO3H][tab-system] ✅ Chip picker setup (${pickerType})`);
    });
  };
      ...
      btn.addEventListener('click', function() {
        if (this.classList.contains('ao3h-reset-confirming')) {
          ...
          if (isFullReset) {
            ...
            console.log('[AO3H][tab-system] Module reset confirmed');
          }
        } else {
  ...
  async function persistSettings(configArea) {
    ...
    document.dispatchEvent(new CustomEvent('ao3h:settingsChanged', {
      bubbles: false,
      ...
      btn.addEventListener('click', function(e) {
        ...
        if (configArea) {
          ...
          if (!isExpanded) {
            ...
            // Inject (?) help buttons after config content is populated
            setTimeout(() => {
              if (typeof window.AO3H_wireHelpButtons === 'function') {
                window.AO3H_wireHelpButtons(configArea);
              }
            }, 0);
          ...
          console.log('[AO3H][tab-system] Config toggled:', isExpanded ? 'closed' : 'opened');
        }
      });
    });
  }
    ...
    moduleRows.forEach(row => {
      ...
      if (checkbox && moduleId) {
        ...
        checkbox.addEventListener('change', async function() {
          ...
          console.log('[AO3H][tab-system]', moduleId, this.checked ? 'enabled' : 'disabled');
        });
      ...
      row.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.ao3h-module-quick-toggle')) return;
...
export function loadTabContent(tabId, tabContainer) {
    ...
    console.log('[AO3H][tab-system] 📥 Loading tab:', tabId);
    ...
    console.log('[AO3H][tab-system] ✅ Tab content loaded:', tabId);
  };
...
export function setupTabSwitching(box) {
    ...
    console.log('[AO3H][tab-system] Setting up tab switching');
    ...
    tabButtons.forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        console.log('[AO3H][tab-system] Switched to tab:', tabId);
      });
    });
    ...
    console.log('[AO3H][tab-system] Loading initial tab:', initialTabId);
