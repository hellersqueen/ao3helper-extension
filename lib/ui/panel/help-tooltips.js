...
export function wireHelpButtons(container) {
  ...
  container.querySelectorAll('.ao3h-setting-item:not([data-help-wired])').forEach((item) => {
    ...
    // Appended as the anchor's last child (not a sibling "afterend") so it
    // flows inline with the label/checkbox text instead of dropping to its
    // own line — .ao3h-setting-label is display:block, so a sibling after it
    // always started a new block box regardless of the button's own
    // display:inline-flex. Safe for the checkbox-<label> anchor too: the
    // click handler's stopPropagation() below already keeps the event from
    // bubbling to the <label>, so this can't double-toggle the checkbox.
