...
export const config = `
                ...
                <div class="ao3h-setting-item">
                    ...
                    <div class="ao3h-setting-control">
                        <input type="number" class="ao3h-config-input" data-setting="staleDays" value="45" min="1">
                    </div>
  ...
  container.addEventListener('click', (e) => {
    ...
    if (restoreBtn) {
      ...
      if (!items.some(i => String(i.wid) === String(wid))) {
        ...
        _lsSaveItems(items);
        // Mirrors _laterShelf.js's addItem(): dispatch so laterShelfCounterBadge
        // stays in sync — restoring from here bypasses addItem() itself.
        document.dispatchEvent(new CustomEvent(EV_MARKED_FOR_LATER, { detail: { workId: item.wid, title: item.title } }));
      }
    }
