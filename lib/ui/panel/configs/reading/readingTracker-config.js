...
function _rtRenderHistoryList (container) {
  ...
  let history = _rtLoadHistory();
  const query = searchInput.value.trim().toLowerCase();
  if (query) {
    history = history.filter(e =>
      (e.title || '').toLowerCase().includes(query) || (e.author || '').toLowerCase().includes(query)
    );
  }
...
function wireConfigArea (container) {
  ...
  if (cleanupBtn && !cleanupBtn.dataset.wired) {
    ...
    cleanupBtn.addEventListener('click', () => {
      ...
      const history = _rtLoadHistory();
      const kept = history.filter(e => e.pinned || (e.lastReadAt || e.seenAt || 0) >= cutoff);
