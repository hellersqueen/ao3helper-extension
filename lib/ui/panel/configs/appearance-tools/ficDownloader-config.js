...
// Prefers the live AO3H.ficDownloader.getOfflineWorks() when the module is
// active on this page — same "check window.AO3H? first, fall back to raw
// storage" pattern as backupAndSync-config.js — so this always reflects
// whatever the module itself currently holds.
export function loadOfflineLibrary (storage = localStorage) {
  const api = window.AO3H?.ficDownloader;
  if (api?.getOfflineWorks) return api.getOfflineWorks();
  try {
...
export function removeOfflineWork (workId, storage = localStorage) {
  const api = window.AO3H?.ficDownloader;
  if (api?.removeOffline) {
    api.removeOffline(workId);
    return api.getOfflineWorks();
  }
...
export const config = `
                ...
                <div id="ao3h-dl-kindle-opts" class="ao3h-indent is-hidden">
                    <div class="ao3h-setting-item">
                ...
                <div id="ao3h-dl-calibre-opts" class="ao3h-indent is-hidden">
                    <div class="ao3h-setting-item">
                ...
                <div class="ao3h-setting-item ao3h-col2">
                    ...
                    <div class="ao3h-setting-control">
                        <input type="number" class="ao3h-config-input ao3h-field--sm" data-setting="maxWorks" value="10" min="1" max="100">
                    </div>
...
function _wireToggle(container, checkboxSelector, optsSelector) {
  ...
  opts.classList.toggle('is-hidden', !checkbox.checked);
