...
function _renderBundleList(container) {
  ...
  for (const bundle of custom) {
    ...
    row.className = 'ao3h-config-row';
    row.style.cssText = 'align-items:flex-start;gap:6px;margin-bottom:4px;';
    row.innerHTML =
      `<div style="flex:1;min-width:0;">`
      + `<strong style="font-size:12px;">${_fmEsc(bundle.name)}</strong>`
      + `<div style="font-size:11px;color:#555;word-break:break-word;margin-top:2px;">${_fmEsc(bundle.tags.join(', '))}</div>`
      + `</div>`
      + `<button class="ao3h-inline-btn ao3h-inline-btn--danger ao3h-fm-bundle-delete"`
      + ` data-bundle-id="${_fmEsc(bundle.id)}" style="flex-shrink:0;">✕</button>`;
...
function wireConfigArea(container) {
  ...
  // Language add button (+ chip container)
  const langAddBtn   = container.querySelector('[data-action="add-language"]');
  const langChips    = container.querySelector('.ao3h-chip-container[data-setting="selectedLanguages"]');
  if (langAddBtn && langChips && !langAddBtn.dataset.wired) {
    langAddBtn.dataset.wired = '1';
    let selected = [];
    try { selected = JSON.parse(langChips.dataset.value || '[]'); } catch {}

    function _addLangChip(name, updateArr) {
      if (updateArr && !selected.includes(name)) selected.push(name);
      const chip = document.createElement('span');
      chip.className = 'ao3h-chip'; chip.dataset.value = name;
      chip.innerHTML = `${_fmEsc(name)} <button type="button" title="Remove">×</button>`;
      chip.querySelector('button').addEventListener('click', () => {
        chip.remove();
        selected = selected.filter(n => n !== name);
        langChips.dataset.value = JSON.stringify(selected);
      });
    ...
    if (selected.length > 0) {
      langChips.innerHTML = '';
      selected.forEach(n => _addLangChip(n, false));
    }

    langAddBtn.addEventListener('click', () => {
      const name = picker?.value;
      if (!name || selected.includes(name)) return;
      _addLangChip(name, true);
      langChips.dataset.value = JSON.stringify(selected);
