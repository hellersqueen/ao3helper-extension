...
export const config = `
                ...
                <div class="ao3h-setting-item ao3h-col2">
                    ...
                    <div class="ao3h-setting-control">
                        <input type="number" class="ao3h-config-input ao3h-field--sm" data-setting="dimOpacity" value="25" min="5" max="90">
                    </div>
                </div>
                ...
                </div><!-- /.ao3h-config-section: Hidden Tags -->
                ...
                </div><!-- /.ao3h-config-section: Whitelist Exceptions -->
                ...
                </div><!-- /.ao3h-config-section: NOPE Words -->
...
// Mirrors createMirroredListStore's norm()/set(): trim, optionally lowercase, dedupe.
function normalizeList(v, { lower = true } = {}) {
  return [...new Set(v.map(s => lower ? s.trim().toLowerCase() : s.trim()).filter(Boolean))];
}

const getHiddenTags    = ()  => lsGet('hideTags', []);
const saveHiddenTags   = (v) => lsSet('hideTags', normalizeList(v));
const getGroupsMap     = ()  => lsGet('hideTagsGroups', {});
// Mirrors HiddenTags.setGroupsMap()'s pruning: drop any group entry whose tag
// isn't (or is no longer) hidden, so removing a tag also clears its group.
const saveGroupsMap    = (m) => {
  const hidden  = new Set(getHiddenTags());
  const cleaned = {};
  for (const [k, v] of Object.entries(m || {})) {
    const key = String(k).toLowerCase();
    if (hidden.has(key)) cleaned[key] = String(v || '').trim();
  }
  lsSet('hideTagsGroups', cleaned);
};
const getEmptyGroups   = ()  => lsGet('hideTagsEmptyGroups', []);
const saveEmptyGroups  = (v) => lsSet('hideTagsEmptyGroups', normalizeList(v, { lower: false }));
const getWhitelistTags = ()  => lsGet('hideTagsWhitelist', []);
const saveWhitelistTags= (v) => lsSet('hideTagsWhitelist', normalizeList(v));
const getNopeWords             = ()  => lsGet('hideTagsNope', []);
const saveNopeWords            = (v) => lsSet('hideTagsNope', normalizeList(v));
...
const saveWhitelistEmptyGroups = (v) => lsSet('hideTagsWhitelistEmptyGroups', normalizeList(v, { lower: false }));
...
const saveNopeEmptyGroups      = (v) => lsSet('hideTagsNopeEmptyGroups', normalizeList(v, { lower: false }));
...
function wireConfigArea(container) {
  ...
  function doImportJSON(onData) {
    pickJSONFile()
      .then(data => { if (data != null) onData(data); })
