...
async function renderConfigList(configArea, query = '') {
  ...
  if (visible.length === 0) {
    listContainer.innerHTML = `<p style="color:#bbb;font-size:11px;font-style:italic;padding:6px 0;margin:0;">No skipped works match “${escapeHtml(query.trim())}”.</p>`;
    return;
  }
...
async function renderNotesList(configArea) {
  ...
  try {
    ...
    container.innerHTML = `<p style="color:#c00;font-size:11px;padding:6px 0;margin:0;">⚠ Could not load private notes (${err?.message || err}).</p>`;
    return;
  }

  const notes = allWorks.filter(w => w.isHidden === false && w.isStandaloneNote === true && (w.reason || '').trim());

  if (notes.length === 0) {
    container.innerHTML = `<p style="color:#bbb;font-size:11px;font-style:italic;padding:6px 0;margin:0;">No private notes yet.</p>`;
    return;
  }
