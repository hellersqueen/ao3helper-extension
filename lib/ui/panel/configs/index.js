...
// ── Styles de config appartenant aux modules ────────────────────────────────
// Le layout de certaines configs (classes ao3h-hbt-*, etc.) vit dans le CSS du
// module, injecté seulement quand le module boote (css() au top-level de son
// fichier). Sur une page où le module ne boote pas, ouvrir Settings affichait un
// panneau non stylé. On injecte donc ces styles depuis le panneau lui-même, sous
// une clé dédiée (panel-cfg-*) indépendante de celle du module, et on les retire
// à la fermeture pour ne pas laisser fuir de règles page (ex. ul.commas li).
import hideByTagsConfigCss from '@modules/browse/hideByTags/hideByTags.css?inline';

const _configStyles = {
  hideByTags: hideByTagsConfigCss,
};

let _configStyleRemovers = [];

/** Injecte le CSS de config des modules dans le panneau (à l'ouverture). */
export function injectConfigStyles() {
  if (_configStyleRemovers.length) return; // déjà injecté
  for (const [moduleId, style] of Object.entries(_configStyles)) {
    if (style) _configStyleRemovers.push(css(style, `panel-cfg-${moduleId}`));
  }
}

/** Retire le CSS de config des modules (à la fermeture du panneau). */
export function removeConfigStyles() {
  for (const remove of _configStyleRemovers) {
    try { remove?.(); } catch { /* noop */ }
  }
  _configStyleRemovers = [];
}
...
log.debug('Panel configs index loaded - ' + listCustomConfigs().length + ' modules registered');
