...
;(function statusTrackerBaseInit(){
  ...
  AO3H.StatusTrackerBase.createCrawler = function(config) {
    ...
    return async function crawlAll({
      ...
      console.log(`[${name}] 🔄 Starting crawl from page ${state.nextPage}`);
      ...
      while (true) {
        ...
        if (state.backoff && nowTs < state.backoff) {
          ...
          console.log(`[${name}] ⏸️  Backoff active, waiting ${Math.ceil(waitMs/1000)}s`);
          await sleep(waitMs + 100);
        }

        // Fetch page
        const url = buildUrl(username, page);
        console.log(`[${name}] 📥 Fetching page ${page}: ${url}`);
        ...
        console.log(`[${name}] ✅ Page ${page}: ${addedThisPage} new IDs (${blurbsSeen} blurbs total)`);
        ...
        if (!hasNext) {
          reachedEnd = true;
          console.log(`[${name}] 🏁 Reached end at page ${page}`);
          break;
        }
        ...
        if (elapsed > maxRuntime) {
          console.log(`[${name}] ⏱️  Runtime limit reached (${elapsed}ms), pausing`);
      ...
      console.log(`[${name}] ✨ Crawl complete: ${totalAdded} new IDs added`);
  ...
  AO3H.StatusTrackerBase.createBadgeRenderer = function(config) {
    ...
    return function renderBadges(cache) {
      ...
      if (addedCount > 0) {
        console.log(`[${name}] ✅ Rendered ${addedCount} badges`);
      }
    };
  };
  ...
  AO3H.StatusTrackerBase.createLiveSync = function(config) {
    ...
    function start() {
      console.log(`[${name}] 🔄 Starting live-sync`);
    ...
    function stop() {
      console.log(`[${name}] ⏹️  Stopping live-sync`);
      if (observer) {
        observer.disconnect();
        observer = null;
      }
  ...
  console.log('[StatusTrackerBase] ✅ Base infrastructure ready');
