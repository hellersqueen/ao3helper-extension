...
const log = getLogger('user-detector');
/* ═══════════════════════════════════════════════════════════════════════════
...
export function detectUser({ fallbackKey = 'AO3H:fallback_username', cachePrefix = null } = {}) {
  ...
  if (manualUser) {
    log.debug('✓ Using manual username:', manualUser);
    return manualUser;
  }
  ...
  if (match && match[1]) {
    log.debug('✓ Username detected from DOM:', match[1]);
    return match[1];
  }
  ...
  if (cachePrefix) {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith(cachePrefix)) {
        ...
        if (userMatch?.[1]) {
          log.debug('✓ Username recovered from cache keys:', userMatch[1]);
          return userMatch[1];
        }
      }
    }
  ...
  log.debug(`→ To set manually: localStorage.setItem('${fallbackKey}', 'YourAO3Username')`);
  return null;
}
