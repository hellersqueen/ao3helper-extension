...
export function getCSRF(doc = document) {
  ...
  const inputToken = /** @type {HTMLInputElement|null} */ (doc.querySelector('input[name="authenticity_token"]'));
...
export async function postAO3Action(workId, action) {
  ...
  for (const sel of selectors) {
    ...
    if (tag === 'form') {
      const formEl = /** @type {HTMLFormElement} */ (el);
      actionUrl = formEl.getAttribute('action') || formEl.action;
      Array.from(formEl.querySelectorAll('input[name]')).forEach(input => {
      ...
      const anchorEl = /** @type {HTMLAnchorElement} */ (el);
      actionUrl = anchorEl.getAttribute('href') || anchorEl.href;
      break;
    }
  }
    ...
    try {
      ...
      if (workResponse.ok) {
        ...
        const form = /** @type {HTMLFormElement|null} */ (
          doc.querySelector(`form.button_to[action*="/works/${workId}/${action}"]`) ||
