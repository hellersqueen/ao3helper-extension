...
export function isListRoute(doc = document) {
  ...
  if (isKudosHistoryPage(path)) return false; // Page kudos history
...
/**
 * Vérifie si on est sur la route virtuelle `/users/<user>/kudos-history`.
 * Cette page n'existe pas nativement sur AO3 (le serveur y répond par un
 * 404) — c'est une route propre à l'extension, rendue par le module
 * ficAppreciation. Définition unique — remplace les copies locales du même
 * regex dans core/lifecycle.js, lib/ui/menu/menu.js et mainNavigation.
 * @param {string} pathname - Chemin à vérifier (défaut: location.pathname)
 * @returns {boolean} True si c'est la page kudos-history
 */
export function isKudosHistoryPage(pathname = location.pathname) {
  return /^\/users\/[^/]+\/kudos-history(?:\/?|$)/.test(pathname);
}
