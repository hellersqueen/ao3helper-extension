...
describe('isKudosHistoryPage — route virtuelle propre à l\'extension', () => {
  it('reconnaît /users/<user>/kudos-history avec ou sans slash final', () => {
    expect(isKudosHistoryPage('/users/alice/kudos-history')).toBe(true);
    expect(isKudosHistoryPage('/users/alice/kudos-history/')).toBe(true);
  });

  it('rejette une autre page user ou une page de work', () => {
    expect(isKudosHistoryPage('/users/alice/bookmarks')).toBe(false);
    expect(isKudosHistoryPage('/works/123')).toBe(false);
  });
});
