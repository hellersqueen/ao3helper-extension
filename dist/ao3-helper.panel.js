...
<div class="ao3h-tab-content" data-tab="about">
    <div style="max-width: 600px; margin: 0 auto; padding: 16px;">
        ...
        <div style="padding: 14px; background: #f9f9f9; border-radius: 8px; margin-bottom: 16px;">
            <p style="color: #555; line-height: 1.6; font-size: 13px; margin: 0;">AO3 Helper enhances your Archive of Our Own experience with ${dt$1.length} modules across ${z$1.length} categories — covering reading, filtering, bookmarking, navigation, appearance, and more.</p>
        </div>
    ...
    `}var ne=Object.fromEntries([...z$1.map(e=>[e.id,C(e)]),[`about`,te()]]);console.log(`[AO3H][panel-tab-content] Tab content generated - ${z$1.length} tabs + About, ${dt$1.length} modules`);var w=`ficEngagement`,re=`
...
`;function mn(e){let t$1=e.querySelector(`[data-action="export-shortcuts"]`);t$1&&!t$1.dataset.wired&&(t$1.dataset.wired=`1`,t$1.addEventListener(`click`,()=>{let e=a(dn),t={};fn.forEach(n=>{n in e&&(t[n]=e[n]);}),n(t,`ao3h-keyboard-shortcuts.json`);}));let r=e.querySelector(`[data-action="import-shortcuts"]`);r&&!r.dataset.wired&&(r.dataset.wired=`1`,r.addEventListener(`click`,t(function*(){let t;try{t=yield i();}catch(e){alert(`Import failed: invalid JSON file.`);return}if(!t||typeof t!=`object`)return;let n={};if(fn.forEach(e=>{typeof t[e]==`string`&&(n[e]=t[e]);}),!Object.keys(n).length){alert(`No recognized shortcut keys found in file.`);return}s(dn,n),Object.entries(n).forEach(([t,n])=>{let r=e.querySelector(`[data-setting="${t}"]`);r&&(r.value=n);}),r.textContent=`✓ Imported ${Object.keys(n).length}`,setTimeout(()=>{r.textContent=`Import`;},1500);})));}var hn=`mainNavigation`,gn=`
...
`,nr=`ao3h:laterShelf:items`,rr=`ao3h:laterShelf:archive`,ir=`ao3h:laterShelf:reminders`,ar=`ao3h:laterShelf:reminders:history`,or=`ao3h:rt:history`,sr=`ao3h:ficAppreciation:status`;function cr(e,t){try{let n=JSON.parse(localStorage.getItem(e)||`null`);return n==null?t:n}catch(e){return t}}function lr(e,t){try{localStorage.setItem(e,JSON.stringify(t));}catch(e){}}function Y(e){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function ur(){let e=cr(nr,[]);return Array.isArray(e)?e:[]}function dr(e){lr(nr,e);}function fr(){let e=cr(rr,[]);return Array.isArray(e)?e:[]}function pr(e){lr(rr,e);}function mr(){let e=cr(ir,{});return e&&typeof e==`object`?e:{}}function hr(){let e=cr(ar,[]);return Array.isArray(e)?e:[]}function gr(){let e=cr(or,[]);return Array.isArray(e)?e:[]}function _r(){let e=cr(sr,{});return e&&typeof e==`object`?e:{}}function vr(e){let t=ur(),n={};t.forEach(e=>{e.group&&(n[e.group]=(n[e.group]||0)+1);});let r=Object.keys(n).sort();if(!r.length){e.innerHTML=`<p class="ao3h-ls-cfg-empty">No groups yet — assign one from the MFL page.</p>`;return}e.innerHTML=r.map(e=>`
    ...
    </div>`).join(``);}function yr(t){t.addEventListener(`click`,n=>{let r=n.target.closest(`[data-action="ls-rename-group"]`),i=n.target.closest(`[data-action="ls-clear-group"]`);if(r){let n=r.dataset.group,i=window.prompt(`Rename group "${n}" to:`,n);if(i===null||!i.trim()||i.trim()===n)return;dr(ur().map(t=>t.group===n?u(u({},t),{},{group:i.trim()}):t)),vr(t);}else if(i){let n=i.dataset.group;if(!window.confirm(`Remove the "${n}" group from all its fics? (the fics themselves stay on the shelf)`))return;dr(ur().map(t=>t.group===n?u(u({},t),{},{group:``}):t)),vr(t);}});}function br(e){let t=fr();if(!t.length){e.innerHTML=`<p class="ao3h-ls-cfg-empty">No removed fics yet.</p>`;return}e.innerHTML=t.slice(0,50).map(e=>`
