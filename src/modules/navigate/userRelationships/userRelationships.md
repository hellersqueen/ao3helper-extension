...
Le module est composé d'un fichier coordinateur, de sept sous-modules fonctionnels, de deux fichiers de logique/réglages partagés et d'une feuille de style.

```text
_userRelationships.js
userRelationshipsSettings.js
userRelationshipsHelpers.js
blockingStats.js
authorTracking.js
authorBlocking.js
authorPreference.js
...
## Motif de blocage ✅ Fait

Permettre d'enregistrer la raison pour laquelle un auteur a été bloqué.

> Raison optionnelle demandée au blocage (menu contextuel ou panneau de
> gestion), stockée dans `userBlocker:reasons`.

---

## Blocage par pseudonyme ✅ Fait

Permettre de bloquer un pseudonyme précis plutôt que l'ensemble du compte d'un auteur.

> Le menu contextuel propose les deux options quand un pseudonyme est
> détecté dans le lien ; un blocage de compte entier couvre tous ses pseudos.

---

## Statistiques d'auteur ❌ Écarté

Afficher le nombre d'œuvres, le nombre total de mots, le nombre de kudos reçus.

> Écarté : nécessiterait de parcourir toutes les pages de l'index d'œuvres
> d'un auteur pour additionner mots/kudos — pas de solution à un seul
> instantané comme pour la détection de nouvelles œuvres.

---

## Œuvres les plus populaires ✅ Fait

Afficher les œuvres les plus populaires d'un auteur.

> Lien "🏆 Sort by kudos" réutilisant le tri natif AO3
> (`work_search[sort_column]=kudos_count`), sans requête supplémentaire.

---

## Historique des kudos ❌ Écarté

Afficher combien de kudos ont été donnés à un auteur au fil du temps.

> Écarté : le stockage des kudos donnés (module Fic Appreciation) ne
> conserve que l'ID du work et la date, pas l'auteur.

---

## Abonnement automatique ❌ Écarté

S'abonner automatiquement aux nouvelles œuvres des auteurs favoris.

> Écarté : confirme la décision déjà prise ("Aucune notification
> automatique"). Même via l'abonnement natif AO3, il faudrait scraper un
> formulaire non documenté sur le profil de l'auteur — fragile.

---

## Priorité de lecture ✅ Fait

Permettre d'attribuer un niveau de priorité aux auteurs pour organiser la liste de lecture.

> Bouton cyclique normal → 🔥 high → 💤 low par auteur.

---

## Statistiques de blocage ✅ Fait

Afficher le nombre d'auteurs bloqués, le nombre total de fics masquées, le nombre total de commentaires masqués.

> Ligne de statistiques dans le panneau de gestion (`blockingStats.js`,
> compteurs cumulés toutes sessions confondues).
...
> Écarté : nécessiterait une requête réseau par auteur distinct affiché
> dans une liste — ne passe pas à l'échelle.

---

## Catégories personnalisées ✅ Fait

Permettre de classer les auteurs avec des tags ou catégories plutôt qu'une simple note libre.

> Champ de tags par auteur (séparés par des virgules), affichés en
> étiquettes, dans `authorPreference.js`.
...
> Nouveau sous-module `authorCard.js`, purement en lecture.
