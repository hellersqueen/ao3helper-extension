...
export class TimelineVisualization {
  /**
   * @param {{ heatmapColor?: string, defaultView?: string, calendarRange?: number, analytics: HistoryAnalytics }} opts
   */
  ...
  _createControls () {
    ...
    monthSel.addEventListener('change', e => { this.currentMonth = parseInt(e.target.value); this._refresh(); });
    ...
    yearSel.addEventListener('change', e => { this.currentYear = parseInt(e.target.value); this._refresh(); });
    const search = document.createElement('input');
    search.type        = 'text';
    search.placeholder = 'Search works (title, author, tags)...';
    search.className = 'ao3h-timeline-search';
    search.addEventListener('input', e => {
      this.filterBySearch(e.target.value, this.analytics.heatmapData, this.currentYear, this.currentView, this.currentMonth);
    });
