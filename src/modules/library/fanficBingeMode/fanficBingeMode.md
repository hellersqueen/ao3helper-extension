...
### `fanficBingeMode.js` — tout le module en un seul fichier
...
### Calculs internes intégrés à `fanficBingeMode.js`
...
- ~~Trois niveaux de priorité dans la file d'attente (haute/moyenne/basse) — en ce moment il n'y a que deux niveaux (normal/haute)~~ ✅ Fait — cycle low/medium/high (⚪/⭐/🌟) au clic sur l'étoile, `fanficBingeMode.js` → `nextPriority()` ; les anciennes entrées `normal` sont traitées comme `medium`
...
> (`fanficBingeMode.js` → `nextPriority()`) ; les anciennes entrées
