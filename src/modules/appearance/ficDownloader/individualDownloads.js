...
AO3 Helper - Fic Downloader › Individual Downloads

Purpose
    Adds a one-click download icon to work blurbs and saves each full work as
    a standalone HTML file containing its summary, notes, and chapters.

Notes
    Buttons are added to existing and dynamically inserted blurbs. Full-work
    content is fetched in one request; active requests and injected controls
    are released when the feature is cleaned up.
...
export class BlurbDownloadButton {
  ...
  init() {
    if (!this.enabled) {
      if (this.logger) {
        this.logger.info('[BlurbDownloadButton] Disabled via showQuickDownloadButtons setting, skipping');
      }
      return;
    }
