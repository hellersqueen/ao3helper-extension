...
AO3 Helper - Chapter Navigation › Blurb Navigation

Adds Start, Continue, and Last Chapter links to work blurbs using saved reading
progress and the coordinator's last-chapter cache.

Notes

- Resume and last-chapter controls are configured independently.
- Reading Tracker's runtime API is preferred, with local storage as fallback.
- Injected button wrappers are removed by `teardown()`.
...
// The coordinator calls `setup()` on listing pages and `teardown()` on cleanup.
