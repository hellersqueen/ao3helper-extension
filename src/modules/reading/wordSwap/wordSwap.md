...
la fic (portée limitée à `#workskin .userstuff`).
...
- Surveille les chapitres chargés dynamiquement (via un `MutationObserver`) pour leur appliquer aussi les règles actives
- Restaure entièrement le texte d'origine quand le module est désactivé ou nettoyé, donc les modifications ne deviennent jamais permanentes
- Réglages stockés dans `localStorage` sous `ao3h:mod:wordSwap:settings` (clé `yourFirstName`) ; les règles elles-mêmes sont stockées à part sous `ao3h:ws:rules`, sous la forme d'un tableau d'objets `{ id, name, find, replace, enabled, regex, caseSensitive, wholeWord, category }`
...
- ~~Des modèles tout prêts pour normaliser automatiquement les variantes d'un nom de personnage~~ ✅
  ...
  cible, une règle regex mot-entier est créée directement par `wordSwap.js`.
