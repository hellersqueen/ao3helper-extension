...
const VOICES = [
  ...
  { name: 'Aria',   lang: 'en-US', localService: false, default: false, voiceURI: 'aria' },
  { name: 'Samira', lang: 'fr-FR', localService: true,  default: false, voiceURI: 'samira' },
];
...
function stubSpeechSynthesis () {
  const listeners = {};
  const synth = {
    speak: vi.fn(u => { synth._lastUtterance = u; }),
    cancel: vi.fn(),
    pause: vi.fn(),
    resume: vi.fn(),
    getVoices: () => VOICES,
...
async function boot (settings = {}) {
  localStorage.setItem('ao3h:mod:textToSpeech:settings', JSON.stringify(settings));
  ...
  await setEnabled('speechEngine', true);
  await setEnabled('contentFiltering', true);
  await setEnabled('visualFeedback', true);
  await setEnabled('playbackControls', true);
  return { synth, setEnabled };
}

async function teardown (setEnabled) {
  await setEnabled('playbackControls', false);
  await setEnabled('visualFeedback', false);
  await setEnabled('contentFiltering', false);
  await setEnabled('speechEngine', false);
  await setEnabled('textToSpeech', false);
}

describe('textToSpeech › playbackControls (intégration)', () => {
  beforeEach(() => {
    localStorage.clear();
    document.body.innerHTML = '';
    vi.restoreAllMocks();
  ...
  it('un profil de vitesse prédéfini met à jour le curseur et son étiquette', async () => {
    ...
    try {
      const comfortableBtn = document.querySelector('.ao3h-tts-preset-btn[data-preset="comfortable"]');
  ...
  it('la fin de chapitre notifie, demande confirmation, et adoucit le passage au chapitre suivant', async () => {
    ...
    try {
      ...
      expect(document.body.classList.contains('ao3h-tts-chapter-fade')).toBe(true);
  ...
  it('le filtre de langue restreint la liste des voix et affiche un indicateur local/réseau', async () => {
    ...
    try {
      ...
      expect([...langSelect.options].map(o => o.value)).toEqual(['', 'en-US', 'fr-FR']);
