...
- Récupère le texte de la fic, ignore facultativement le résumé et les notes,
  puis le découpe en phrases.
- Possède le moteur Web Speech, la sélection de voix et la création des
  phrases prononcées.
- Construit le panneau principal : lecture, pause, navigation, vitesse,
  volume, hauteur, minuterie et passage au chapitre suivant.
- Garde les ressources obligatoires sous un seul cycle de vie : elles
  démarrent et s'arrêtent toujours avec Text To Speech.
- Expose l'API partagée avant le démarrage des deux options enfants.

### 2. `pronunciationManager.js` — corriger la prononciation
...
### 3. `visualFeedback.js` — suivre la lecture des yeux
...
- ~~Régler le volume ou la hauteur de la voix~~ ✅ Fait — curseurs Volume et Pitch dans le panneau flottant et le panneau de réglages (`_textToSpeech.js` applique `utterance.volume`/`utterance.pitch`)
...
faux aujourd'hui : elle est bel et bien codée dans `_textToSpeech.js`
