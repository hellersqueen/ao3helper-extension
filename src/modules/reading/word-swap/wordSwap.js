...
// ── Y/N shortcut — inject synthetic rule at list front ────────────────────
...
function renderRulesPanel(container) {
    ...
    function renderList(rules) {
        ...
        // Wire: toggle enable
        ul.querySelectorAll('.ao3h-ws-toggle').forEach(cb => {
        ...
        // Wire: delete
        ul.querySelectorAll('.ao3h-ws-delete').forEach(btn => {
        ...
        ul.querySelectorAll('.ao3h-ws-edit').forEach(btn => {
            btn.addEventListener('click', () => {
                const r = loadRules();
        ...
        // Wire: preview
        const inp = prevSec.querySelector('.ao3h-ws-preview-input');
    ...
    function openRuleForm(existing, onSave) {
        ...
        modal.querySelector('.ao3h-ws-f-cancel').onclick = () => modal.remove();
        modal.querySelector('.ao3h-ws-f-save').onclick = () => {
            const updated = {
                ...rule,
                name:          modal.querySelector('.ao3h-ws-f-name').value.trim(),
                find:          modal.querySelector('.ao3h-ws-f-find').value,
                replace:       modal.querySelector('.ao3h-ws-f-replace').value,
                category:      modal.querySelector('.ao3h-ws-f-cat').value.trim(),
                caseSensitive: modal.querySelector('.ao3h-ws-f-cs').checked,
                wholeWord:     modal.querySelector('.ao3h-ws-f-ww').checked,
                regex:         modal.querySelector('.ao3h-ws-f-rx').checked,
            };
