...
| Élément | Où | Verdict |
|---|---|---|
| Bascule de classes CSS sur `<html>` pilotée par un `classMap` | ~~`statsVisibility.js`, `datesTimestamps.js` (jumeaux à 95 %), + toggles simples dans `minimalHeader`, `statsOnChaptersList`~~, `hoverReveal`, `compactModeTags` (browse) | **Fait (2026-07-20)** — les 4 fichiers visual-preferences fusionnés en une seule classe `VisibilityToggles` (`visibilityToggles.js`), le pattern étant strictement identique entre eux (même state, même cycle de vie). `hoverReveal` (CSS pur) et `compactModeTags` (browse, module non encore audité) restent inchangés. |
